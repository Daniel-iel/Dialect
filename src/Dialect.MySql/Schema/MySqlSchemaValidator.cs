namespace Dialect.MySql.Schema;
...
/// <summary>
/// MySQL-specific schema validator (8.0+).
/// Enforces MySQL naming conventions, identifier limits (64 chars),
/// data type compatibility, and constraint rules.
/// </summary>
public class MySqlSchemaValidator : SchemaValidator
{
    // MySQL reserved keywords (common subset)
    private static readonly HashSet<string> ReservedKeywords = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        "index", "on", "unique", "null", "default", "auto_increment", "values", "set",
        "grant", "revoke", "execute", "function", "procedure", "return", "begin", "end",
        "if", "else", "loop", "while", "repeat", "case", "when", "then", "order", "by",
        "group", "having", "distinct", "all", "any", "exists", "in", "like", "between",
        "union", "intersect", "except", "cast", "convert", "substring", "concat", "length"
    };
        ...
        if (ReservedKeywords.Contains(name))
            return ValidationResult.WithWarnings(
                new ValidationWarning(ValidationRules.ReservedKeywordUsed,
                    $"Migration name '{name}' is a reserved MySQL keyword",
                    name));
