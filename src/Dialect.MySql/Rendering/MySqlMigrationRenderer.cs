...
/// <summary>
/// Renders migration steps to MySQL SQL.
/// </summary>
public sealed class MySqlMigrationRenderer : IMigrationRenderer
{
    ...
    private string RenderAlterColumn(AlterColumnStep step, ISqlDialect dialect)
    {
        ...
        if (step.NewType.HasValue)
        {
            ...
            var nullable = step.Nullable.HasValue ? (step.Nullable.Value ? "" : " NOT NULL") : "";
            var defaultClause = step.DefaultValue != null ? $" DEFAULT {FormatValue(step.DefaultValue)}" : "";
            statements.Add($"ALTER TABLE {tableName} MODIFY COLUMN {columnName} {type}{nullable}{defaultClause}");
        }
        else if (step.Nullable.HasValue || step.DefaultValue != null)
        {
            var nullable = step.Nullable.HasValue ? (step.Nullable.Value ? "" : " NOT NULL") : "";
            var defaultClause = step.DefaultValue != null ? $" DEFAULT {FormatValue(step.DefaultValue)}" : "";
            // For MySQL, we need the type even if only modifying nullable/default
            // This shouldn't happen in practice as you'd always provide the type
            throw new InvalidOperationException("MySQL requires specifying the column type when modifying nullable or default values");
        }
    ...
    private string RenderColumnDefinition(ColumnDef column, ISqlDialect dialect)
    {
        ...
        var autoIncrement = column.IsAutoIncrement ? " AUTO_INCREMENT" : "";
        var defaultClause = column.DefaultValue != null && !column.IsAutoIncrement ? $" DEFAULT {FormatValue(column.DefaultValue)}" : "";

        return $"{columnName} {type}{autoIncrement}{nullable}{defaultClause}".Trim();
    }
    ...
    private string GetDataTypeSql(DataType type, ISqlDialect dialect)
    {
        return type switch
        {
            ...
            DataType.DateTime2 => "DATETIME(6)",
            DataType.SmallDateTime => "DATETIME",
            DataType.Timestamp => "TIMESTAMP DEFAULT CURRENT_TIMESTAMP",
    ...
    {
        // MySQL uses ` for identifier quoting
        return $"`{identifier}`";
    }
